# Awesome AI Gadgets [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

> A curated list of AI-powered gadgets, devices, and hardware for consumers, developers, and makers.

## Contents

- [Consumer AI Devices](#consumer-ai-devices)
  - [AI Assistants & Companions](#ai-assistants--companions)
  - [Smart Glasses & Wearables](#smart-glasses--wearables)
  - [Translation Devices](#translation-devices)
  - [AI Cameras](#ai-cameras)
- [Developer Hardware](#developer-hardware)
  - [AI Development Boards](#ai-development-boards)
  - [Edge AI Accelerators](#edge-ai-accelerators)
  - [Robotics Platforms](#robotics-platforms)
- [Smart Home AI](#smart-home-ai)
  - [Voice Assistants](#voice-assistants)
  - [Security & Monitoring](#security--monitoring)
  - [Home Automation](#home-automation)
- [Educational & DIY Kits](#educational--diy-kits)
- [Recommended AI Gadget Combos by Budget](#recommended-ai-gadget-combos-by-budget)
- [Resources](#resources)

## Consumer AI Devices

### AI Assistants & Companions

- **[Rabbit R1](https://www.rabbit.tech/)** - $199 - Pocket AI companion with natural language interface and camera
- **[Humane AI Pin](https://humane.com/)** - $699 - Wearable AI assistant with laser projection display
- **[Friend](https://friend.com/)** - $99 - Wearable AI companion focused on emotional support
- **[Rewind Pendant](https://www.rewind.ai/pendant)** - $59 - AI pendant that records and transcribes conversations
- **[Plaud NotePin](https://www.plaud.ai/)** - $169 - AI-powered wearable voice recorder with automatic transcription and summarization

### Smart Glasses & Wearables

- **[Ray-Ban Meta Smart Glasses](https://www.meta.com/smart-glasses/)** - $299+ - Smart glasses with AI assistant, camera, and speakers
- **[XREAL Air 2](https://www.xreal.com/)** - $399 - AR glasses with AI-powered spatial computing
- **[Frame by Brilliant Labs](https://brilliant.xyz/)** - $349 - Open-source AI glasses with multimodal AI
- **[Solos AirGo3](https://solosglasses.com/)** - $249 - Smart glasses with ChatGPT integration

### Translation Devices

- **[Pocketalk Plus](https://pocketalk.com/)** - $299 - Real-time voice translation device supporting 82 languages
- **[Timekettle WT2 Edge](https://www.timekettle.co/)** - $349 - AI-powered translation earbuds
- **[Vasco V4](https://vasco-translator.com/)** - $299 - Handheld translator with photo translation
- **[ili Wearable Translator](https://iamili.com/)** - $199 - Offline wearable translator

### AI Cameras

- **[Obsbot Tail Air](https://www.obsbot.com/)** - $179 - AI-powered streaming camera with auto-tracking
- **[Insta360 X3](https://www.insta360.com/)** - $449 - 360° camera with AI editing features
- **[DJI Pocket 2](https://www.dji.com/)** - $349 - Handheld gimbal camera with AI tracking
- **[Pixie Smart Camera](https://getpixie.ai/)** - $99 - AI camera for automated content creation

## Developer Hardware

### AI Development Boards

- **[NVIDIA Jetson Nano](https://developer.nvidia.com/embedded/jetson-nano)** - $99 - Entry-level AI development board
- **[NVIDIA Jetson Orin Nano](https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-orin/)** - $499 - Advanced AI compute platform
- **[Google Coral Dev Board](https://coral.ai/products/dev-board)** - $149 - Development board with Edge TPU
- **[Raspberry Pi 5](https://www.raspberrypi.com/products/raspberry-pi-5/)** - $60+ - Popular SBC with AI capabilities
- **[Orange Pi 5](http://www.orangepi.org/)** - $89+ - RK3588S-based AI development board

### Edge AI Accelerators

- **[Google Coral USB Accelerator](https://coral.ai/products/accelerator)** - $59 - USB Edge TPU coprocessor
- **[Intel Neural Compute Stick 2](https://www.intel.com/content/www/us/en/developer/articles/tool/neural-compute-stick.html)** - $69 - USB AI accelerator
- **[Hailo-8 AI Processor](https://hailo.ai/)** - Contact for pricing - High-performance edge AI processor
- **[Luxonis OAK-D](https://luxonis.com/)** - $199 - Spatial AI camera with onboard processing

### Robotics Platforms

- **[NVIDIA Jetson AGX Orin Developer Kit](https://developer.nvidia.com/embedded/jetson-agx-orin-developer-kit)** - $1999 - Professional robotics AI platform
- **[Qualcomm RB5 Development Kit](https://developer.qualcomm.com/hardware/rb5-development-kit)** - $499 - 5G + AI robotics platform
- **[OpenCV AI Kit](https://opencv.org/opencv-ai-kit/)** - $149 - Computer vision development platform

## Smart Home AI

### Voice Assistants

- **[Amazon Echo (4th Gen)](https://www.amazon.com/dp/B07XKF5RM3)** - $99 - Alexa-powered smart speaker
- **[Google Nest Hub Max](https://store.google.com/product/nest_hub_max)** - $229 - Smart display with Google Assistant
- **[Apple HomePod mini](https://www.apple.com/homepod-mini/)** - $99 - Siri-powered smart speaker
- **[Sonos One](https://www.sonos.com/en-us/shop/one)** - $219 - Multi-assistant smart speaker

### Security & Monitoring

- **[Arlo Pro 4](https://www.arlo.com/)** - $199 - AI-powered security camera
- **[Ring Video Doorbell Pro 2](https://ring.com/)** - $249 - Smart doorbell with 3D motion detection
- **[Eufy Security S330](https://www.eufylife.com/)** - $549 - AI facial recognition security system
- **[Wyze Cam v3](https://www.wyze.com/)** - $35 - Budget AI-powered security camera

### Home Automation

- **[Hubitat Elevation](https://hubitat.com/)** - $149 - Local AI-powered home automation hub
- **[Home Assistant Blue](https://www.home-assistant.io/)** - $139 - Open-source AI home automation
- **[SmartThings Hub](https://www.smartthings.com/)** - $124 - Samsung's AI-enabled smart home hub

## Educational & DIY Kits

- **[Google AIY Projects](https://aiyprojects.withgoogle.com/)** - $24-89 - DIY AI kits for makers
- **[DFRobot HuskyLens](https://www.dfrobot.com/product-1922.html)** - $44 - AI vision sensor for education
- **[M5Stack UnitV2](https://shop.m5stack.com/products/unitv2-ai-camera)** - $65 - Compact AI camera module
- **[Seeed Studio XIAO ESP32S3 Sense](https://www.seeedstudio.com/)** - $13 - Tiny AI-capable microcontroller
- **[SparkFun Edge Development Board](https://www.sparkfun.com/)** - $24 - TensorFlow Lite development board

## Recommended AI Gadget Combos by Budget

### Budget Starter Pack (~$200)
- **Raspberry Pi 5** ($60) + **Google Coral USB Accelerator** ($59) + **DFRobot HuskyLens** ($44) = $163
- Perfect for: Learning AI development, computer vision projects, home automation

### Smart Home Essential (~$400)
- **Amazon Echo (4th Gen)** ($99) + **Wyze Cam v3** x3 ($105) + **SmartThings Hub** ($124) = $328
- Perfect for: Complete smart home setup with voice control and security

### Content Creator Kit (~$600)
- **DJI Pocket 2** ($349) + **Obsbot Tail Air** ($179) + **Pixie Smart Camera** ($99) = $627
- Perfect for: Streamers, vloggers, and social media creators

### Mobile AI Professional (~$800)
- **Ray-Ban Meta Smart Glasses** ($299) + **Pocketalk Plus** ($299) + **Plaud NotePin** ($169) = $767
- Perfect for: Business travelers, journalists, international professionals

### Developer Powerhouse (~$1,000)
- **NVIDIA Jetson Orin Nano** ($499) + **Luxonis OAK-D** ($199) + **Google Coral Dev Board** ($149) + **Intel Neural Compute Stick 2** ($69) = $916
- Perfect for: Serious AI development, robotics, computer vision projects

### Ultimate AI Lifestyle (~$2,000)
- **Humane AI Pin** ($699) + **XREAL Air 2** ($399) + **Timekettle WT2 Edge** ($349) + **Insta360 X3** ($449) = $1,896
- Perfect for: Early adopters wanting the full AI-powered lifestyle experience

### Education Classroom Kit (~$500)
- **Google AIY Projects** x5 ($445) + **Seeed Studio XIAO ESP32S3 Sense** x10 ($130) = $575
- Perfect for: Teaching AI/ML concepts to a classroom of students

## Resources

- [Edge Impulse](https://www.edgeimpulse.com/) - Platform for developing AI for edge devices
- [TensorFlow Lite](https://www.tensorflow.org/lite) - Framework for on-device inference
- [OpenCV](https://opencv.org/) - Computer vision library
- [Hugging Face](https://huggingface.co/) - AI model repository and tools

## Contributing

Your contributions are always welcome! Please read the [contribution guidelines](CONTRIBUTING.md) first.

## License

[![CC0](https://licensebuttons.net/p/zero/1.0/88x31.png)](https://creativecommons.org/publicdomain/zero/1.0/)